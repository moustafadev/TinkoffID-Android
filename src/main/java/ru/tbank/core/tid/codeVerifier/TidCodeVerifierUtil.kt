 ...
 */
internal object TidCodeVerifierUtil {
    ...
    fun deriveCodeVerifierChallenge(codeVerifier: String): String {
        val codeVerifierBytes: ByteArray = try {
            codeVerifier.toByteArray(Charsets.ISO_8859_1)
        } catch (e: UnsupportedEncodingException) {
            throw IllegalStateException("ISO-8859-1 encoding not supported", e)
        }

        val sha256Digester = try {
            MessageDigest.getInstance("SHA-256")
        } catch (e: NoSuchAlgorithmException) {
            null
        }

        val hashBytes = if (sha256Digester != null) {
            sha256Digester.update(codeVerifierBytes)
            sha256Digester.digest()
        } else {
            try {
                SHA256Utils.hash(codeVerifierBytes)
            } catch (e: RuntimeException) {
                null
            }
        }

        return hashBytes?.let { bytes ->
            Base64Encoder.encodeToString(bytes, PKCE_BASE64_ENCODE_SETTINGS)
        } ?: codeVerifier
    }
